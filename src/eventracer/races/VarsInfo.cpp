...
class RaceGraph {
public:
	RaceGraph(const VarsInfo& vars, const EventGraphInterface& graph) :
	...
	void buildTopGraph() {
		...
		for (size_t j = 0; j < m_topRaces.size(); ++j) {
			const VarsInfo::Race_Info& rj = getRace(j);
			for (size_t i = j + 1; i < m_topRaces.size(); ++i) {
				const VarsInfo::Race_Info& ri = getRace(i);
	...
	void checkCoverage(VarsInfo::AllRaces* all_races) {
		...
		for (size_t j = 0; j < m_topRaces.size(); ++j) {
			if (isMultiCovered(j,
					&(*all_races)[m_topRaces[j]].m_multiParentRaces)) {
				++numMultiCovered;
			}
		}
	...
	bool hasPathViaRaces(int node1, int node2, int cmd_in_node2,
		...
		if (node1 > node2)
			return false;
		if (m_graph.areOrdered(node1, node2))
			return true;
		...
		while (!q.empty()) {
			int currId = q.front();
			q.pop();
			const VarsInfo::Race_Info& curr = getRace(currId);
			if (curr.m_event2 > node2)
				continue;
			if (!curr.canSynchronizeInThisOrder())
				continue;

			if ((curr.m_event2 == node2 && curr.m_cmdInEvent2 < cmd_in_node2)
					|| (curr.m_event2 < node2
							&& m_graph.areOrdered(curr.m_event2, node2))) {
				while (currId >= 0) {
...
private:
	...
	const VarsInfo::Race_Info& getRace(int topId) const {
	...
	bool isMultiCovered(int raceId, std::vector<int>* covered_by) const {
		const VarsInfo::Race_Info& race = getRace(raceId);
		return hasPathViaRaces(race.m_event1, race.m_event2, race.m_cmdInEvent2,
				covered_by);
	}
...
VarsInfo::VarsInfo() :
		m_startTime(0), m_timedOut(false), m_timeToFindRacesMs(0), m_numChains(
				0), m_fastEventGraph(NULL), m_raceGraph(NULL) {
}
		...
		for (int i = 0; i < static_cast<int>(data.m_accesses.size()); ++i) {
			...
			if (!currAccess.m_isRead) {
				...
				if (last_read_id != -1) {
					...
					if (!m_fastEventGraph->areOrdered(lastRead.m_eventActionId,
...
void VarsInfo::findRaces(const ActionLog& actions,
	...
	for (int i = 0; i < graph.numNodes(); ++i) {
		...
		if (!graph.nodeSuccessors(i).empty()
				&& !graph.nodePredecessors(i).empty()) {
			++m_numNodes;
		}
	}
...
//			cout<<"Var: "<<m_vars.getString()<<"\n";

			const VarAccess& currAccess = data.m_accesses[i];
			if (last_write_id != -1) {
				const VarAccess& lastWrite = data.m_accesses[last_write_id];
//				cout<<"Last write: "<<lastWrite.m_eventActionId<<endl;
//				cout<<"Curr read : "<<currAccess.m_eventActionId<<endl;
				if (!m_fastEventGraph->areOrdered(lastWrite.m_eventActionId,
						currAccess.m_eventActionId)) {
//					if (true) {
					// A write-write or write-read race was detected.
					m_races.push_back(
							Race_Info(data.getVarAccessTypeForId(last_write_id),
									data.getVarAccessTypeForId(i),
									lastWrite.m_eventActionId,
									currAccess.m_eventActionId,
									lastWrite.m_commandIdInEvent,
									currAccess.m_commandIdInEvent, it->first));

//					cout << "Race count: " << m_races.size() << endl;
...
//				if (currAccess.m_isRead) {
					...
					m_races.push_back(
							Race_Info(data.getVarAccessTypeForId(i),
									data.getVarAccessTypeForId(last_write_id),
									currAccess.m_eventActionId,
									lastWrite.m_eventActionId,
									currAccess.m_commandIdInEvent,
									lastWrite.m_commandIdInEvent, it->first));
//					cout << "Race count: " << m_races.size() << endl;
					++data.m_numRWRaces;
				}
			}
	...
	cout << "At last: Race count: " << m_races.size() << endl;
	printf("Has %d vars with WW races, %d with RW and %d with WR.\n", vars_ww,
			vars_rw, vars_wr);

	findRaceDependency(actions);

	cout << "At last2: Race count: " << m_races.size() << endl;
	printf("Race size: %ld\n", m_races.size());

//
//	for (AllRaces::iterator it = m_races.begin(); it != m_races.end(); ++it) {
//		cout<<"Race Info for latest replay"<<endl;
//		cout<<"Var is:  "<<(*it).m_varId<<endl;
//		cout<<"Event1:  "<<(*it).m_event1<<endl;
//		cout<<"Event2:  "<<(*it).m_event1<<endl;
//		cout<<endl;
//
//
//
...
//	for (AllRaces::iterator it = m_races.begin(); it != m_races.end(); ++it) {
//
//		(*it).m_event1=RaceHandler::Instance()->findRootOfNestedEvent((*it).m_event1);
//		(*it).m_event2=RaceHandler::Instance()->findRootOfNestedEvent((*it).m_event2);
//
//		int from = (*it).m_event1;
//		int to = (*it).m_event2;
//
//
//
...
//			static int count = 1;
//			cout << "----------------------------------------------------\n";
////			cout << "Race: " << from << " VS " << to << "\n";
//			(*it).dump();
//			stringstream ss;
////
//			ss << "----------------------------------------------------\n";
//			ss << "New race: " << from << " , " << to << ",  count: " << count
//					<< endl;
//
//
//
...
//			}
//
////			int e1 = RaceHandler::Instance()->getLastTimerEventId(from);
////			int e2 = RaceHandler::Instance()->getLastTimerEventId(to);
//
////			if (from != -1 && to != -1) {
////				RaceHandler::Instance()->recordReplayTraceNew(from, to);
////
////			}
////			else{
////				stringstream ss2;
////				if(from==-1){
////					ss2<<"Couldn't find predecessor for From: "<<from<<endl;
////				}
////				if(to==-1){
////					ss2<<"Couldn't find predecessor for To: "<<from<<endl;
////				}
////				if (RaceHandler::Instance()->WhetherGerneateRaceLogs) {
////					RaceHandler::Instance()->logToFile("race", ss2.str());
////				}
////				else{
////					RaceHandler::Instance()->logToFile("race_no", ss2.str());
////				}
////
////			}
//			count++;
...
////			static int count = 1;
////			stringstream ss;
////			ss << "Skip covered race: " << from << " , " << to << ",  count: "
////					<< count << endl;
////			RaceHandler::Instance()->logToFile("race", ss.str());
////			count++;
//		}
...
class Op2IsAfter {
public:
	bool operator()(const VarsInfo::Race_Info& r1,
			const VarsInfo::Race_Info& r2) const {
		if (r1.m_event2 == r2.m_event2)
			return r1.m_cmdInEvent2 < r2.m_cmdInEvent2;
		return r1.m_event2 < r2.m_event2;
	}
};

typedef std::pair<VarsInfo::Race_Info, int> RaceAndPos;

class Op2IsAfterPair {
public:
	bool operator()(const RaceAndPos& r1, const RaceAndPos& r2) const {
		if (r1.first.m_event2 == r2.first.m_event2) {
			if (r1.first.m_cmdInEvent2 == r2.first.m_cmdInEvent2)
...
bool VarsInfo::shouldTimeout() {
	if (FLAGS_race_detection_timeout_seconds != 0) {
		bool result = (GetCurrentTimeMicros() - m_startTime)
				> FLAGS_race_detection_timeout_seconds * 1000000;
		if (result) {
			m_timedOut = true;
...
// Create remapping : old position of a race to new position.
	...
	for (AllVarData::iterator var_it = m_vars.begin(); var_it != m_vars.end();
			++var_it) {
...
void VarsInfo::getDirectRaceChildren(int race_id,
		bool only_different_event_actions,
		std::set<int>* direct_child_races) const {
	const Race_Info& base_race = m_races[race_id];

	std::vector<int> base_race_direct_children;
	for (size_t i = race_id + 1; i < m_races.size(); ++i) {
		const Race_Info& race2 = m_races[i];

		if (only_different_event_actions && base_race.m_event1 == race2.m_event1
				&& base_race.m_event2 == race2.m_event2) {
			continue;
		}
...
// base_race being a synchronization could prevent race2.
		if (m_fastEventGraph->areOrdered(base_race.m_event2, race2.m_event2)
			...
			for (size_t j = 0; j < base_race_direct_children.size(); ++j) {
				const Race_Info& race1 = m_races[base_race_direct_children[j]];
				if (!race1.canSynchronizeInThisOrder())
					continue;
				// race1 being a synchronization could prevent race2.
				if (m_fastEventGraph->areOrdered(race1.m_event2, race2.m_event2)
...
const char* VarsInfo::Race_Info::TypeStr() const {
	switch (m_access1) {
	case MEMORY_READ: {
		switch (m_access2) {
		case MEMORY_READ:
			return "READ-READ";  // Should never happen.
		case MEMORY_WRITE:
			return "READ-WRITE";
		case MEMORY_UPDATE:
			return "READ-UPDATE";
...
int VarsInfo::getCommandIdForVarReadInEventAction(const VarData& var,
		int event_action_id) {
	for (size_t i = 0; i < var.m_accesses.size(); ++i) {
		if (var.m_accesses[i].m_eventActionId == event_action_id
				&& var.m_accesses[i].m_isRead) {
...
int VarsInfo::getCommandIdForVarWriteInEventAction(const VarData& var,
		int event_action_id) {
	for (size_t i = 0; i < var.m_accesses.size(); ++i) {
		if (var.m_accesses[i].m_eventActionId == event_action_id
				&& !var.m_accesses[i].m_isRead) {
...
VarsInfo::VarAccessType VarsInfo::getVarAccessTypeInEventAction(
		const VarsInfo::VarData& var, int event_action_id) {
	int write_cmd = getCommandIdForVarWriteInEventAction(var, event_action_id);
	if (write_cmd == -1) {
		return VarsInfo::MEMORY_READ;
	} else {
		int read_cmd = getCommandIdForVarReadInEventAction(var,
				event_action_id);
...
const char* VarsInfo::Race_Info::AccessStr(VarAccessType access) {
	switch (access) {
	case MEMORY_READ:
		return "READ";
	case MEMORY_WRITE:
