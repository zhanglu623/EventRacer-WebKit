...
public:
	...
	struct VarAccess {
		...
		int64 traceOrder() const {
			return (static_cast<int64>(m_eventActionId) << 32)
					+ m_commandIdInEvent;
		}
	};
	...
	struct VarData {
		VarData() :
				m_numWWRaces(0), m_numWRRaces(0), m_numRWRaces(0) {
		}
		...
		VarAccessType getVarAccessTypeForId(int access_index) const {
			if (m_accesses[access_index].m_isRead) {
				for (int i = access_index + 1;
						i < static_cast<int>(m_accesses.size()); ++i) {
					if (m_accesses[i].m_eventActionId
							!= m_accesses[access_index].m_eventActionId)
						break;
		...
		const VarAccess* findAccessLocation(bool is_read,
				int event_action_id) const {
			for (size_t i = 0; i < m_accesses.size(); ++i) {
				if (m_accesses[i].m_eventActionId == event_action_id
						&& m_accesses[i].m_isRead == is_read) {
					return &m_accesses[i];
				}
			}
			return NULL;
	...
	const AllVarData& variables() const {
		return m_vars;
	}

	// Returns the command id for a read or a write of a variable in an event action.
	// If there is no such command id, returns -1.
	static int getCommandIdForVarReadInEventAction(const VarData& var,
			int event_action_id);
	static int getCommandIdForVarWriteInEventAction(const VarData& var,
			int event_action_id);
	...
	static VarAccessType getVarAccessTypeInEventAction(const VarData& var,
			int event_action_id);
	...
	struct Race_Info {
		...
		Race_Info(VarAccessType a1, VarAccessType a2, int e1, int e2,
				int command_in_e1, int command_in_e2, int v) :
				m_access1(a1), m_access2(a2), m_event1(e1), m_event2(e2), m_cmdInEvent1(
						command_in_e1), m_cmdInEvent2(command_in_e2), m_varId(
						v), m_coveredBy(-1) {
//			printf("----------------------------\n");
//			printf("|Race:  E1: %d   vs   E2: %d\n",e1,e2);
	...
	typedef std::vector<Race_Info> AllRaces;

	const AllRaces& races() const {
		return m_races;
	}

	// Appends the set of direct child races of a race to the given set.
	void getDirectRaceChildren(int race_id, bool only_different_event_actions,
