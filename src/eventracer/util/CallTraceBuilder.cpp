...
void CallTraceBuilder::Init(
	...
	for (size_t i = 0; i < arcs.size(); ++i) {
		if (arcs[i].m_duration > 0) {
			sorted_arcs.push_back(std::pair<int, int>(arcs[i].from, arcs[i].to));
		}
	}
...
void CallTraceBuilder::Lu_getCallTraceOfAllCommand(int event_action_id, int command_id, std::vector<int>* scope) const {
	scope->clear();
	if (event_action_id < 0 || event_action_id >= static_cast<int>(m_parentScope.size())) return;
	const std::vector<int>& parents = m_parentScope[event_action_id];
	int cmd = command_id;
	int i=0;
	for (i=0; i < static_cast<int>(parents.size()); i++) {
		//if (cmd > parents[cmd]) printf("Error\n");
		cmd = parents[i];
		if (cmd >= 0) scope->push_back(cmd);
	}
