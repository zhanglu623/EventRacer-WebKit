...
EventGraphFixer::EventGraphFixer(ActionLog* log, StringSet* vars,
		StringSet* scopes, SimpleDirectedGraph* event_graph,
		EventGraphInfo* graph_info) :
		m_log(log), m_vars(vars), m_scopes(scopes), m_eventGraph(event_graph), m_graphInfo(
				graph_info) {
}
...
void EventGraphFixer::dropNoFollowerEmptyEvents() {
	...
	for (int i = m_eventGraph->numNodes(); i > 0;) {
		--i;
		int temp1 = m_eventGraph->nodeSuccessors(i).size();
		int temp2 = m_log->event_action(i).m_commands.size();
//		printf("-------------------------------\n");
//		printf("Successors: %d,  Commands: %d\n", temp1, temp2);
		if (temp1 == 0 && temp2 == 0) {
...
void EventGraphFixer::addScriptsAndResourcesHappensBefore() {
	...
	for (int op_id = 0; op_id <= m_log->maxEventActionId(); ++op_id) {
		if (m_eventGraph->isNodeDeleted(op_id))
			continue;
		...
		for (size_t i = 0; i < op.m_commands.size(); ++i) {
			...
			if (cmd.m_cmdType == ActionLog::WRITE_MEMORY
				...
				if (getScriptOrResourceRunnerString(
						m_vars->getString(cmd.m_location), &script_id)) {
					std::map<std::string, int>::const_iterator it =
							m_lastLoc.find(script_id);
					if (it != m_lastLoc.end()) {
...
void EventGraphFixer::makeIndependentEventExploration() {
	...
	for (int event_action_id = 0; event_action_id <= m_log->maxEventActionId();
		...
		if (m_eventGraph->isNodeDeleted(event_action_id))
			continue;
		const ActionLog::EventAction& op = m_log->event_action(event_action_id);
		if (op.m_type != ActionLog::USER_INTERFACE)
			continue;
		if (op.m_commands.size() > 0
				&& op.m_commands[0].m_cmdType == ActionLog::ENTER_SCOPE
				&& strcmp(m_scopes->getString(op.m_commands[0].m_location),
						"auto:explore") == 0) {
