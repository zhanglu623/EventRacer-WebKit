...
public:
	bool operator()(const ActionLog::Arc& a1, const ActionLog::Arc& a2) const {
		if (a1.from == a2.from) return a1.to < a2.to;
...
TimerGraph::TimerGraph(const std::vector<ActionLog::Arc>& arcs, const SimpleDirectedGraph& graph) {
	...
	for (size_t i = 0; i < arcs.size(); ++i) {
		if (arcs[i].m_duration >= 0 &&
				!graph.isNodeDeleted(arcs[i].to) &&
				!graph.isNodeDeleted(arcs[i].from)) {
			m_timedArcs.push_back(arcs[i]);
			++num_timed_arcs;
//			cout<<"From: "<<arcs[i].from<<"  To: "<<arcs[i].to<<"  Duration: "<<arcs[i].m_duration<<"\n";
		}
	}
...
void TimerGraph::build(SimpleDirectedGraph* graph) {
	...
	for (size_t arci = 0; arci < m_timedArcs.size(); ++arci) {
		...
		while (it.readNoAddFollowers(&node_id)) {
			...
			if (min_outgoing_duration[node_id] <= arc.m_duration) {
				...
				for (size_t i = node_arcs.size(); i > 0;) {
					...
					if (it.isVisited(prev_arc.to)) continue;
					if (prev_arc.m_duration <= arc.m_duration) {
						...
						if (covered_arcs.count(prev_arc.to) == 0) {
							num_added_arcs += graph->addArcIfNeeded(prev_arc.to, arc.to);
						}
						covered_arcs.insert(
								graph->nodePredecessors(prev_arc.to).begin(),
								graph->nodePredecessors(prev_arc.to).end());
					}
		...
		min_outgoing_duration[arc.from] = std::min(
				min_outgoing_duration[arc.from],
				arc.m_duration);
		outgoing_arc_indices[arc.from].push_back(arci);
	}
