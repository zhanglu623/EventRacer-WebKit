...
public:
	...
	void addRaceArc(int race_id, const VarsInfo::Race_Info& race, const char* color);
private:
	...
	struct Race {
		Race(int id, const VarsInfo::Race_Info& info, const char* color) : m_id(id), m_varInfo(&info), m_color(color) {}
		int m_id;
		const VarsInfo::Race_Info* m_varInfo;
		const char* m_color;
	};
