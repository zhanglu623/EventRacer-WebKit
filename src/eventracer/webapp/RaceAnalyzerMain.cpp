...
//	request_path="/hb";
	...
	mg_printf(conn,
			"HTTP/1.1 200 OK\r\n"
					...
					"\r\n"
					"%s",
			request_path.find(".gif") != std::string::npos ?
					"image/gif" : "text/html", static_cast<int>(reply.size()),
			reply.c_str());
...
int main(int argc, char* argv[]) {
	...
	const char *options[] = { "listening_ports", FLAGS_port.c_str(),
			"document_root", FLAGS_dot_temp_dir.c_str(), NULL };
	...
	if (argc < 2) {
		fprintf(stderr,
				"One must specify an input ER_actionlog file\nUsage %s <file>\n",
				argv[0]);
		return 1;
	}

	bool exitWhenGetRaces=false;
	for (int i = 0; i < argc; ++i) {

		if(strcmp(argv[i], "exitWhenGetRaces") == 0)
		{
			exitWhenGetRaces=true;
		}
		printf("%d: %s\n", i, argv[i]);
	}
	...
	if (argc >= 3) {
		...
		if (strcmp(argv[2], "no") == 0)
			RaceHandler::Instance()->WhetherGerneateRaceLogs = false;
		printf("Do not generate race replay logs\n");
	}
//	if (argc >= 4) {
//		RaceInfo::Instance()->readToken(argv[3]);
//	}

	// Creating a race app.
	race_app = new RaceApp(0, argv[1]);
	printf("Total races: %d, replable: %d.\n", race_app->m_vinfo.m_races.size(),race_app->replayableRacesCount);
	if(exitWhenGetRaces){
	...
	printf(
			"Web server started on port %s. Open http://localhost:%s/ in your browser...\n",
